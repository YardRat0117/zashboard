...
export const connections = computed(() => {
    return connectionTabShow.value === CONNECTION_TAB_TYPE.ACTIVE ? activeConnections.value : closedConnections.value
})
...
export const renderConnections = computed(() => {
    ...
    return connections.value
        .filter((conn) => {
            ...
            if (sourceIPFilter.value !== null && sourceIPFilter.value.every((i) => i !== conn.metadata.sourceIP)) {
                return false
            }
