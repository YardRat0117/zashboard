    ...
    <div
        ...
        <div class="card mx-auto w-96 max-w-[90%] gap-3 px-6 py-2 max-sm:my-4">
            ...
            <div class="flex flex-col gap-1">
                ...
                <select class="select select-sm w-full" v-model="form.protocol">
            ...
            <div class="flex flex-col gap-1">
                ...
                <TextInput class="w-full" name="username" autocomplete="username" v-model="form.host" />
            </div>
            <div class="flex flex-col gap-1">
                <label class="text-sm">
                    <span>{{ $t('port') }}</span>
                </label>
                <TextInput class="w-full" v-model="form.port" />
            </div>
            <div class="flex flex-col gap-1">
                <label class="flex items-center gap-1 text-sm">
                    <span>{{ $t('secondaryPath') }} ({{ $t('optional') }})</span>
                    <span class="tooltip" :data-tip="$t('secondaryPathTip')">
                        <QuestionMarkCircleIcon class="h-4 w-4" />
                    </span>
                </label>
                <TextInput class="w-full" v-model="form.secondaryPath" />
            </div>
            <div class="flex flex-col gap-1">
                <label class="text-sm">
                    <span>{{ $t('password') }}</span>
                </label>
                <input type="password" class="input input-sm w-full" v-model="form.password" />
            </div>
            <div class="flex flex-col gap-1">
                <label class="text-sm">
                    <span>{{ $t('label') }} ({{ $t('optional') }})</span>
                </label>
                <TextInput class="w-full" v-model="form.label" />
            </div>
            <button class="btn btn-primary btn-sm w-full" @click="handleSubmit(form)">
                {{ $t('submit') }}
            </button>
            <Draggable
                ...
                <template #item="{ element }">
                    <div :key="element.uuid" class="flex items-center gap-2">
                        <button class="btn btn-circle btn-ghost btn-sm">
                            <ChevronUpDownIcon class="h-4 w-4 cursor-grab" />
                        </button>
                        <button class="btn btn-sm flex-1" @click="selectBackend(element.uuid)">
                            {{ getLabelFromBackend(element) }}
                        </button>
                        <button class="btn btn-circle btn-ghost btn-sm" @click="editBackend(element)">
                            <PencilIcon class="h-4 w-4" />
                        </button>
                        <button class="btn btn-circle btn-ghost btn-sm" @click="() => removeBackend(element.uuid)">
        ...
        <!-- 编辑Backend Modal -->
        <EditBackendModal v-model="showEditModal" :default-backend-uuid="editingBackendUuid" />
    </div>
</template>
