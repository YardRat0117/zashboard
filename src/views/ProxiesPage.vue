    ...
    <div
        ...
        <template v-if="displayTwoColumns">
            <div class="grid grid-cols-2 gap-1 p-2 md:pr-1">
                <div v-for="idx in [0, 1]" :key="idx" class="flex flex-1 flex-col gap-1">
                    <component
        ...
        <div class="grid grid-cols-1 gap-1 p-2 md:pr-1" v-else>
            <component v-for="name in renderGroups" :is="renderComponent" :key="name" :name="name" />
        </div>
    </div>
</template>
...
const waitTickUntilReady = (startTime = performance.now()): void => {
    if (
