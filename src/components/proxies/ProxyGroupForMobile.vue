<template>
    <div class="relative h-22 cursor-pointer" ref="cardWrapperRef" @click="handlerGroupClick">
        <div v-if="modalMode" class="bg-base-300/50 fixed inset-0 z-40 overflow-hidden" />
        <div
            ...
            <div class="relative flex h-22 shrink-0 flex-col justify-between p-2">
                <div class="text-md truncate" :class="proxyGroup.icon && 'pr-10'">
                    {{ proxyGroup.name }}
                </div>
                <div class="text-base-content/60 truncate text-xs" :class="proxyGroup.icon && 'pr-10'">
                ...
                <div class="flex items-center">
                    <div class="flex flex-1 items-center gap-1 truncate">
                        <button
                            ...
                            <EyeIcon v-if="!hiddenGroup" class="h-3 w-3" />
                            <EyeSlashIcon v-else class="h-3 w-3" />
                        </button>
                        <ProxyGroupNow :name="proxyGroup.name" :mobile="true" />
                    </div>
                    <LatencyTag
...
const calcCardStyle = () => {
    requestAnimationFrame(() => {
        ...
        const transformOrigin = isLeft ? (isTop ? 'top left' : 'bottom left') : isTop ? 'top right' : 'bottom right'
