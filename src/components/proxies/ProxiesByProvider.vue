...
const groupedProxies = computed(() => {
    ...
    for (const proxy of props.renderProxies) {
        ...
        const providerName =
            ...
            (proxyProviderList.value.find((group) => group.proxies.find((node) => node.name === proxy))?.name ?? '')
...
<template>
    <div class="flex flex-col gap-2">
        <div v-for="({ providerName, proxies }, index) in truncatedProxies" :key="index">
            <p class="my-2 text-sm font-semibold" v-if="providerName !== ''">
                {{ providerName }}
            </p>
            <ProxyNodeGrid>
