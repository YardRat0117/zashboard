...
import { getCloudflareLatencyAPI, getDockerLatencyAPI, getGithubLatencyAPI, getGoogleLatencyAPI } from '@/api/latency'
import { cloudflareLatency, dockerLatency, githubLatency, googleLatency } from '@/composables/overview'
...
onMounted(() => {
    if (
        ...
        [googleLatency, cloudflareLatency, githubLatency, dockerLatency].some((item) => item.value === '')
    ) {
        getLatency()
    }
})
