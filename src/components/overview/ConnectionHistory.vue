<template>
    <div class="card w-full backdrop-blur-none!">
        <div class="card-title need-blur flex items-center justify-between px-4 pt-4">
            <div class="flex w-full items-center gap-4 max-sm:flex-col max-sm:items-start">
                <div class="flex flex-1 items-center gap-2">
                    ...
                    <button class="btn btn-circle btn-sm" @click="showClearDialog = true">
                ...
                <div class="flex items-center gap-2 font-normal max-sm:flex-col max-sm:items-start">
                    <div class="flex items-center gap-2">
                        ...
                        <select v-model="aggregationType" class="select select-bordered select-sm w-32">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <select v-model="autoCleanupInterval" class="select select-bordered select-sm w-28">
                            <option :value="AutoCleanupInterval.Week">
                                {{ $t('autoCleanupIntervalWeek') }}
                            </option>
                ...
                <table class="table-sm table-zebra table w-full rounded-none">
                        ...
                        <tr>
                            <th
                                ...
                                <div class="flex items-center gap-1">
                                    ...
                                    <ArrowUpCircleIcon v-if="header.column.getIsSorted() === 'asc'" class="h-4 w-4" />
                                    <ArrowDownCircleIcon
                    ...
                    <tbody>
                        <tr
                            ...
                            <td v-for="cell in rows[virtualRow.index].getVisibleCells()" :key="cell.id" class="text-sm">
        ...
        <DialogWrapper v-model="showClearDialog" :title="$t('clearConnectionHistory')">
            <div class="flex flex-col gap-4 p-2">
                <p class="text-sm">
                    {{ $t('clearConnectionHistoryConfirm') }}
                </p>
                <div class="flex justify-end gap-2">
                    <button class="btn btn-sm" @click="showClearDialog = false">
                        {{ $t('cancel') }}
                    </button>
                    <button class="btn btn-error btn-sm" @click="handleClearHistory">
                        {{ $t('confirm') }}
                    </button>
                </div>
            </div>
...
import { ArrowDownCircleIcon, ArrowUpCircleIcon, QuestionMarkCircleIcon, TrashIcon } from '@heroicons/vue/24/outline'
import {
    FlexRender,
    getCoreRowModel,
    getSortedRowModel,
...
const sorting = useStorage<SortingState>('cache/connection-history-sorting', [{ id: 'download', desc: true }])
