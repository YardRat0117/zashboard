<template>
    <div class="relative h-48 w-full overflow-hidden xl:h-64">
        ...
        <span
            ...
            "
            ref="colorRef" />
        <div v-if="barData.length === 0" class="text-base-content/50 absolute inset-0 flex items-center justify-center">
            <div class="text-center">
                <div>{{ t('noData') }}</div>
            </div>
        </div>
        <button class="btn btn-ghost btn-xs absolute right-1 bottom-0" @click="isPaused = !isPaused">
