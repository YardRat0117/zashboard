<template>
    <div class="card w-full" v-if="hasProvidersWithTraffic">
        ...
        <div
            ...
            ">
            <!-- 总流量 -->
            <div class="bg-base-200/50 flex flex-col gap-2 rounded-lg p-2" v-if="hasMultipleProvidersWithTraffic">
            ...
            <div
                ...
                <div class="text-base-content/60 flex items-center justify-between text-sm">
                    <div>
                        {{ $t('remainingTraffic') }}:
                        {{ provider.remainingStr }}
                    </div>
                    <div>
                        {{ $t('usedTraffic') }}: {{ provider.usedStr }} /
                        {{ provider.totalStr }}
                    </div>
                </div>
            </div>
    ...
    return proxyProviderList.value
        ...
        .map((provider) => {
            ...
            return {
                ...
                total: Total,
                percentage,
                usedStr: prettyBytesHelper(used, {
                    binary: true,
                }),
                remainingStr: prettyBytesHelper(remaining, {
                    binary: true,
                }),
                totalStr: prettyBytesHelper(Total, {
                    binary: true,
                }),
            }
        })
...
const totalUsedStr = computed(() => {
    return prettyBytesHelper(totalTraffic.value.used, {
        binary: true,
    })
})

const totalRemainingStr = computed(() => {
    return prettyBytesHelper(totalTraffic.value.remaining, {
        binary: true,
    })
})

const totalTotalStr = computed(() => {
    return prettyBytesHelper(totalTraffic.value.total, {
        binary: true,
    })
})
