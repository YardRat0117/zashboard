<template>
    <button class="btn btn-sm" @click="importDialogShow = true">
        {{ $t('importSettings') }}
    </button>
    <DialogWrapper v-model="importDialogShow" :title="$t('importSettings')">
        <div class="my-4 flex items-center gap-2">
            {{ $t('importFromFile') }}
            <button class="btn btn-sm" @click="importSettingsFromFile">
                {{ $t('importFromFile') }}
        ...
        <div class="my-4 flex items-center gap-2 max-sm:flex-col max-sm:items-start">
            ...
            <div class="flex items-center gap-2">
                <div class="join">
                    <TextInput v-model="importSettingsUrl" class="max-w-60" />
                    <button class="btn btn-sm join-item" @click="importSettingsFromUrlHandler()">
                ...
                <QuestionMarkCircleIcon
                    ...
                    " />
                <button v-else class="btn btn-sm" @click="importSettingsUrl = DEFAULT_SETTINGS_URL">
                    {{ $t('reset') }} URL
                </button>
            </div>
        </div>
        <div class="my-4 flex items-center gap-2">
            <label class="flex cursor-pointer items-center gap-2">
                ...
                <input v-model="autoImportSettings" type="checkbox" class="toggle toggle-sm" />
            </label>
            <QuestionMarkCircleIcon
                class="h-4 w-4"
                @mouseenter="
        ...
        <input ref="inputRef" type="file" accept=".json" class="hidden" @change="handlerJsonUpload" />
    </DialogWrapper>
</template>
