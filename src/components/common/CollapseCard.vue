<template>
    <div :class="`collapse ${showCollapse ? 'collapse-open' : 'collapse-close'}`">
        <div class="collapse-title cursor-pointer overflow-hidden pr-4" @click="showCollapse = !showCollapse">
            <slot name="title" />
            <slot v-if="!showCollapse" name="preview" />
        </div>
        <div class="collapse-content max-sm:px-2" @transitionend="handlerTransitionEnd">
            <div
                v-if="showContent"
