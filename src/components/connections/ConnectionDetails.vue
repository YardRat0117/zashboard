<template>
    <DialogWrapper
        ...
        <div class="flex flex-col md:flex-row">
            <div class="md:w-128">
                <VueJsonPretty :data="infoConn" class="overflow-y-auto">
                    <template #renderNodeValue="{ node, defaultValue }">
                        <template v-if="node.path.startsWith('root.chains') && proxyMap[node.content]?.icon">
                            <span>
                                "
                                <ProxyIcon :icon="proxyMap[node.content].icon" class="inline-block" :margin="0" />
                                {{ node.content }}"
                            </span>
                        </template>
                        <template v-else>
                ...
                <div class="min-h-12 shrink-0 pt-2 text-sm" v-if="destinationIP && !isPrivateIP">
