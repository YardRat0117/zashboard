        ...
        const downloadAllLogs = (): void => {
            const blob = new Blob(
                [
                    logs.value
                        .map((log) =>
                            [log.seq.toString().padEnd(5, ' '), log.time, log.type.padEnd(7, ' '), log.payload].join(
                                '\t',
                            ),
                        )
                        .join('\n'),
        ...
        return (): VNode<RendererNode, RendererElement, { [key: string]: undefined }> => {
            ...
            const settingsModal = (
                <>
                    ...
                    <DialogWrapper v-model={settingsModel.value} title={t('logSettings')}>
                        <div class="flex flex-col gap-4 p-2 text-sm">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <TextInput class="w-32 max-w-64 flex-1" v-model={logFilterRegex.value} />
                            </div>
                            <div class="flex items-center gap-2">
                                {t('hideLog')}
                                <input type="checkbox" class="toggle" v-model={logFilterEnabled.value} />
                                <div
                                    onMouseenter={(e) =>
            ...
            const buttons = (
                <div class="flex items-center gap-2">
                    {settingsModal}
                    <button class="btn btn-circle btn-sm" onClick={downloadAllLogs}>
                    ...
                    <button
                        ...
                        {logFilterEnabled.value ? <LinkSlashIcon class="h-4 w-4" /> : <LinkIcon class="h-4 w-4" />}
                    </button>
                    <button class="btn btn-circle btn-sm" onClick={() => (isPaused.value = !isPaused.value)}>
                        {isPaused.value ? <PlayIcon class="h-4 w-4" /> : <PauseIcon class="h-4 w-4" />}
                    </button>
                    <button class="btn btn-circle btn-sm" onClick={() => (logs.value = [])}>
