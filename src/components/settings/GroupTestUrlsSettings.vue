<template>
    <div class="mb-2 flex items-center gap-2">
        ...
        <button v-if="groupTestUrls.length" class="btn btn-sm btn-circle" @click="dialogVisible = !dialogVisible">
            <ChevronUpIcon v-if="dialogVisible" class="h-4 w-4" />
            <ChevronDownIcon v-else class="h-4 w-4" />
        </button>
        <QuestionMarkCircleIcon class="h-4 w-4" @mouseenter="groupTestUrlsTip" />
    </div>
    <div class="transparent-collapse collapse rounded-none shadow-none" :class="dialogVisible ? 'collapse-open' : ''">
        <div class="collapse-content p-0">
            <div class="grid grid-cols-1 gap-2">
                <template v-if="dialogVisible">
                    <div v-for="groupTestUrl in groupTestUrls" :key="groupTestUrl.uuid" class="flex items-center gap-2">
                        <TextInput
                            class="w-32"
                        ...
                        <button class="btn btn-sm btn-circle" @click="removeGroupTestUrl(groupTestUrl.uuid)">
    ...
    <div class="flex items-center gap-2">
        ...
        <TextInput
            ...
            :placeholder="$t('speedtestUrl')"
            @keydown.enter="addGroupTestUrl" />
        <button class="btn btn-sm btn-circle" @click="addGroupTestUrl">
