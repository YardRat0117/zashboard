<template>
    <div class="join w-96 max-sm:w-full">
        <TextInput v-model="form.name" placeholder="Domain Name" :clearable="true" />
        <TextInput v-model="form.type" class="w-28" placeholder="Type" :menus="['A', 'AAAA', 'HTTPS']" />
        <button class="btn join-item btn-sm" @click="query">
            {{ $t('DNSQuery') }}
        </button>
    </div>
    <div class="flex max-h-96 flex-col gap-1 overflow-y-auto">
        <div class="flex gap-1" v-for="item in resultList" :key="item.data">
            <div>{{ item.name }}</div>
            :
            <div>{{ item.data }}</div>
        </div>
    </div>
    <div v-if="details" class="flex gap-1">
        <div class="mr-3 flex items-center gap-1" v-if="details?.country">
            <MapPinIcon class="h-4 w-4 shrink-0" />
            <template v-if="details?.city && details?.city !== details?.country">{{ details?.city }},</template>
