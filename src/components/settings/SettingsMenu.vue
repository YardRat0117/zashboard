<template>
    <div ref="menuRef" class="settings-menu scrollbar-hidden ctrls-bar p-1 px-2">
        <div class="max-w relative flex w-full flex-row">
            ...
            <button class="btn btn-circle btn-sm my-auto" @click="showVisibilityDialog = true">
...
const { isSwiping } = useSwipe(menuRef, {
    ...
    onSwipe(e: TouchEvent) {
        ...
        activeLeft.value = Math.max(
            0,
            Math.min(relativeX - activeWidth.value / 2, menuRef.value.offsetWidth - activeWidth.value - 16),
        )
