<template>
    <div class="flex items-center gap-2">
        ...
        <button v-if="iconReflectList.length" class="btn btn-sm btn-circle" @click="dialogVisible = !dialogVisible">
        ...
        <div class="collapse-content p-0">
            <div class="grid grid-cols-1 gap-2 md:grid-cols-2">
                <template v-if="dialogVisible">
                    <div v-for="iconReflect in iconReflectList" :key="iconReflect.uuid" class="flex items-center gap-2">
                        <TextInput class="w-32" v-model="iconReflect.name" :placeholder="$t('groupName')" />
                        <ArrowRightCircleIcon class="h-4 w-4 shrink-0" />
                        <TextInput v-model="iconReflect.icon" placeholder="Icon URL" />
                        <button class="btn btn-sm btn-circle" @click="removeIconReflect(iconReflect.uuid)">
    ...
    <div class="flex items-center gap-2">
        <TextInput
            ...
            placeholder="Name"
            :menus="proxyGroupList.filter((group) => !iconReflectList.some((item) => item.name === group))"
            @keydown.enter="addIconReflect" />
        <ArrowRightCircleIcon class="h-4 w-4 shrink-0" />
        <TextInput v-model="newIconReflect.icon" placeholder="Icon URL" @keydown.enter="addIconReflect" />
        <button class="btn btn-sm btn-circle" @click="addIconReflect">
