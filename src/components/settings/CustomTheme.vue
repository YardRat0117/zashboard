<template>
    <DialogWrapper v-model="model" :title="$t('customTheme')">
            ...
            <div v-for="color in colors" :key="color">
                <label class="flex cursor-pointer flex-col items-center gap-1">
                    ...
                    <div
                        ...
                        :style="`background-color: ${customTheme[color as keyof typeof customTheme]};`"></div>
                    <input
        ...
        <div class="grid grid-cols-1 gap-2 md:grid-cols-3">
            <div v-for="radius in radiusKey" :key="radius" class="flex items-center gap-2">
                {{ radius.replace('--radius-', '') }}
                <TextInput class="w-20" v-model="customTheme[radius as keyof typeof customTheme] as string" />
            </div>
        </div>
        <div class="divider">Size</div>
        <div class="grid grid-cols-1 gap-2 md:grid-cols-3">
            <div v-for="size in sizeKey" :key="size" class="flex items-center gap-2">
                {{ size.replace('--size-', '') }}
                <TextInput class="w-20" v-model="customTheme[size as keyof typeof customTheme] as string" />
            </div>
            <div class="flex items-center gap-2">
                border
                <TextInput class="w-20" v-model="customTheme['--border']" />
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-2 pb-12 md:grid-cols-3">
            <div>
                depth
                <input class="toggle" v-model="depth" type="checkbox" />
            </div>
            <div>
                noise
                <input class="toggle" v-model="noise" type="checkbox" />
            </div>
            <div>
                dark
                <input class="toggle" v-model="dark" type="checkbox" />
            </div>
        </div>
        <div class="bg-base-100 border-base-200 absolute right-0 bottom-0 left-0 flex gap-2 border-t p-2 pt-2">
            <select class="select select-sm w-26" v-model="applyFrom">
                <option v-for="opt in ALL_THEME" :key="opt" :value="opt">
                    {{ opt }}
                </option>
            </select>
            <button class="btn btn-sm" @click="resetCustomTheme">
                {{ $t('reset') }}
            </button>
            <div class="flex-1"></div>
            <a class="btn btn-sm" href="https://daisyui.com/theme-generator/" target="_blank">
                {{ $t('moreDetails') }}
            </a>
            <button class="btn btn-sm btn-primary" @click="handlerCustomThemeSave">
                {{ $t('save') }}
            </button>
        </div>
    </DialogWrapper>
...
const customTheme = reactive<THEME>({
    ...(customThemes.value[0] || DEFAULT_THEME),
})
