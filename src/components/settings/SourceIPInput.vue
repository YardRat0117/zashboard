<template>
    <div class="relative flex w-full items-center gap-2" :class="sourceIPLabel.scope?.length ? 'pt-4' : ''">
        <slot name="prefix"></slot>
        <span class="absolute top-0 left-6 truncate text-xs" @mouseenter="checkTruncation">
            {{
                backendList
        ...
        <div
            ...
            <LockClosedIcon v-if="isLocked" class="h-4 w-4" />
            <LockOpenIcon v-else class="h-4 w-4" />
        </div>
        <ArrowRightCircleIcon class="h-4 w-4 shrink-0" />
        <TextInput class="w-24 sm:w-40" v-model="sourceIPLabel.label" :placeholder="$t('label')" />
        <slot></slot>
    </div>
</template>
