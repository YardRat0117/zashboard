<template>
    <div class="flex flex-col gap-3">
        ...
        <div class="flex items-center gap-2">
            <button class="btn btn-sm" @click="((connectionCardLines = SIMPLE_CARD_STYLE), setRestOfColumns())">
                {{ $t('simpleCardPreset') }}
            </button>
            <button class="btn btn-sm" @click="((connectionCardLines = DETAILED_CARD_STYLE), setRestOfColumns())">
        ...
        <div class="relative flex flex-col rounded-sm">
            ...
            <div :class="`p-2 ${connectionCardLines.length % 2 === 1 ? 'bg-base-300' : 'bg-base-200'}`">
                <button class="btn btn-circle bg-base-100 hover:bg-base-200 btn-sm shadow-sm" @click="addLine">
